...
void die(unsigned char response){ //if the response is different than OK, print and loop forever
  if(response!=OK){
    lcd.string(10,row,LCDWIDTH-1,LCDHEIGHT-1,"Error... forever loop @",0);
    while(1);  
  }
}
...
void loop() { //main loop 
    ...
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"FAT file read,write demo!",0);                  row+=10;
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Open File <testfile.txt>",0); row+=10;
    
    //try to open the file
    res=lcd.SDFopenFile("testfile.txt", WRITEONLY);  //Try to open a file called "testFile.txt" in write only mode
    if(res!=OK){                       //If the file doesn't Open is because it doesn't exist      
      lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-File not exist, create file...",0); row+=10;    
      res=lcd.SDFnewFile("testfile.txt");  //Try to create the file 
      res|=lcd.SDFopenFile("testfile.txt", WRITEONLY);  //Try to open the created file      
      die(res);                        //If any error loop forever
    }
    
    //Up to here the file exist and is open    
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-File Open in WRITE mode...",0); row+=10;    
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Writting... ",0); row+=10;    
    ...
    die(res);                         //If any error loop forever    
    lcd.SDFsaveFile();                //Save changes in the file contained in WORKSPACE0
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Closing File...",0); row+=10;    
    lcd.SDFcloseFile();               //Close the file --------------------
    
    //Now lets verify contents
    //open again the file in read only mode
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Open File <testfile.txt>",0); row+=10;    
    res=lcd.SDFopenFile("testfile.txt", READONLY);  //Try to open again the file read only mode
    die(res);                         //If any error loop forever
    //read the file  
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-File Open in READ mode...",0); row+=10;    
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Read bytes to buffer...",0); row+=10;    
    ...
    //check contents
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Verify/Compare contents...",0); row+=10;            
    for(i=0;i<sizeof(message);i++){      
      if(message[i]!=buffer[i]){ //if contents are different
        lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Contents differ, erase file",0); row+=10;                  
        lcd.SDFeraseFile("testfile.txt"); //erase the File
        while(1);
      }
    }
    lcd.string(0,row,LCDWIDTH-1,LCDHEIGHT-1,"-Contents are equal...END",0);                     
    while(1);
}
