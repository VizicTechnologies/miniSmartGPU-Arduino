...
void updateHorizontalBar(unsigned int slValue){ //updates/draws a #defined horizontal bar with the received value from 0 - SLH_WIDTH
  ...
  lcd.drawRectangle(SLH_X_START -1, SLH_Y_START -1, SLH_X_START + SLH_WIDTH +1, SLH_Y_START + SLH_HEIGHT +1, SG_WHITE, SG_UNFILL);
  //erase the slider - BLACK
  lcd.drawRectangle(SLH_X_START + slValue, SLH_Y_START, SLH_X_START + SLH_WIDTH, SLH_Y_START + SLH_HEIGHT, SG_BLACK, SG_FILL);    
  //draw the current slider value - GREEN
  lcd.drawRectangle(SLH_X_START, SLH_Y_START, SLH_X_START + slValue, SLH_Y_START + SLH_HEIGHT, SG_GREEN, SG_FILL);       
}
...
void updateVerticalBar(unsigned int slValue){ //updates/draws a #defined vertical bar with the received value from 0 - SLV_HEIGHT
  ...
  //draw the slider WHITE corner, we substract 1 and add 1 to make it a little bigger than the desired slider size
  lcd.drawRectangle(SLV_X_START -1, SLV_Y_START -1, SLV_X_START + SLV_WIDTH +1, SLV_Y_START + SLV_HEIGHT +1, SG_WHITE, SG_UNFILL);
  //erase the slider - BLACK
  lcd.drawRectangle(SLV_X_START, SLV_Y_START, SLV_X_START + SLV_WIDTH, SLV_Y_START + SLV_HEIGHT - slValue, SG_BLACK, SG_FILL);  
  //draw the current slider value - YELLOW
  lcd.drawRectangle(SLV_X_START, SLV_Y_START + SLV_HEIGHT - slValue, SLV_X_START + SLV_WIDTH, SLV_Y_START + SLV_HEIGHT, SG_YELLOW, SG_FILL);      
}
