...
void initGame(void){
    ...
    NUM_SPRITES = 42;
    tile_size = 10;
    NUM_LINES = (LCD_HEIGHT+1) / tile_size;      //+1
    NUM_COLUMNS = (LCD_WIDTH+1) / tile_size-1; //+1
    ball_x_offset = 0;
    ball_y_offset = 0;
    platform_x_offset = 0;
    level = 1;
    ...
    // screen
    NUM_TILES_X = (LCD_WIDTH) / tile_size;
    NUM_TILES_Y = (LCD_HEIGHT) / tile_size;
    lcd.erase();
}

// display a single tile
void display_tile(int tile, int x, int y){
    lcd.drawRectangle(x, y, x + tile_size-1, y + tile_size-1, SG_BLACK, SG_FILL);
}
...
void display_sprite(int i){
    ...
    if (i == 0){// platform
        lcd.drawRectangle(Sprite_x[i] * tile_size + (tile_size / 4), Sprite_y[i] * tile_size, (Sprite_x[i] + 2) * tile_size - (tile_size / 4), (Sprite_y[i] + 1) * tile_size - (tile_size / 2), SG_WHITE,SG_FILL);
        lcd.drawCircle(Sprite_x[i]*tile_size+tile_size/4, Sprite_y[i]*tile_size+tile_size/4,tile_size/4,SG_WHITE,SG_FILL);
        lcd.drawCircle((Sprite_x[i] + 2) * tile_size - (tile_size / 4), Sprite_y[i]*tile_size+tile_size/4,tile_size/4,SG_WHITE,SG_FILL);
    }else if (i == 1){//ball
        lcd.drawCircle(Sprite_x[i]*tile_size+tile_size/2+ball_x_offset, Sprite_y[i]*tile_size+tile_size/2+ball_y_offset,tile_size/2 - 1,SG_WHITE,SG_FILL);
        ...
        if (i < 42){
            lcd.drawRectangle(Sprite_x[i] * tile_size, Sprite_y[i] * tile_size, (Sprite_x[i] + 2) * tile_size - 1, (Sprite_y[i] + 1) * tile_size - 1, SG_BLACK,SG_UNFILL);
            lcd.drawRectangle(Sprite_x[i] * tile_size, Sprite_y[i] * tile_size, (Sprite_x[i] + 2) * tile_size - 2, (Sprite_y[i] + 1) * tile_size - 2, SG_WHITE,SG_UNFILL);
            lcd.drawRectangle(Sprite_x[i] * tile_size + 1, Sprite_y[i] * tile_size + 1, (Sprite_x[i] + 2) * tile_size - 2, (Sprite_y[i] + 1) * tile_size - 2, colour,SG_FILL);
        }
...
void welcome_screen(){
  ...
  lcd.string(0,0,LCD_WIDTH-1,LCD_HEIGHT-1,"***********************",0);
  lcd.string(10,20,LCD_WIDTH-1,LCD_HEIGHT-1,"Vizic Technologies",0);  
  lcd.string(22,50,LCD_WIDTH-1,LCD_HEIGHT-1,"--ARKANOID--",0);     
  lcd.string(23,80,LCD_WIDTH-1,LCD_HEIGHT-1,"mini SmartGPU",0);     
  lcd.string(0,112,LCD_WIDTH-1,LCD_HEIGHT-1,"***********************",0);  
...
void loop() { //main loop    
    ...
    while(1){
        ...
        // game over
        lcd.drawRectangle(0,115,LCD_WIDTH-1,LCD_HEIGHT-1,SG_BLACK,SG_FILL);
        if (Sprite_count <= 0){
            level++;
            lcd.string(45,80,LCD_WIDTH-1,LCD_HEIGHT-1,"You Win!",0);            
        }else{
            lcd.string(43,80,LCD_WIDTH-1,LCD_HEIGHT-1,"You Lose!",0);            
        }
        while(digitalRead(LEFT)!=0 && digitalRead(RIGHT)!=0); //while no button is pressed
    }
}
