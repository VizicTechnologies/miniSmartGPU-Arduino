...
void pong(){
  ...
  lcd.drawRectangle(0,15,159,127,SG_MAGENTA,SG_UNFILL);                        //draw corners
  lcd.drawLine(bar,122,bar+barSize,122,SG_WHITE);                           //draw Bar 
    ...
    lcd.string(3,2,30,30,buffer,0);                                      //display current points
      for(i=0;i<5;i++){                                                  //check 5 times if the player touches the screen
        if(!(digitalRead(MOVELEFT) & digitalRead(MOVERIGHT))){           //if a button is pressed then we move the bar to pressed button side
          lcd.drawLine(bar,122,bar+barSize,122,SG_BLACK);                   //erase previous Bar    
          ...
          lcd.drawLine(bar,122,bar+barSize,122,SG_WHITE);                   //draw the new bar at the new position
        }
      }
      //we update the balls
...
void loop() { //main loop
  ...
  while(1){                // Loop forever
    //Draw a cover
    lcd.drawTriangle(12,10,40,20,25,40,SG_YELLOW,SG_FILL);
    lcd.drawRectangle(120,15,145,40,SG_MAGENTA,SG_FILL);
    lcd.drawCircle(28,100,14,SG_CYAN,SG_FILL);
    lcd.drawTriangle(130,115,115,95,145,85,SG_RED,SG_FILL);
    lcd.setTextBackFill(SG_TRANS);
