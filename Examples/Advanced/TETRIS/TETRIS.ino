...
int shape_can_move (int s, int dir_x, int dir_y){
    ...
    while(block<4){
        ...
        }else if (screen[shape_pos_x + (TET_MAX_BLOCKS_X * shape_pos_y)] != BLACK){
            pass = 0;
        }
        if (pass == 0){
            break;
...
void remove_from_screen (int s){
    ...
    while (block<4){
        shape_pos_x = origin_x[s] + offsets_x[(s * 4) + block];
        shape_pos_y = origin_y[s] + offsets_y[(s * 4) + block];
        screen[shape_pos_x + (TET_MAX_BLOCKS_X * shape_pos_y)] = BLACK;
        block++;
    }
}
  ...
  int found_lines, x, line;
    ...
    while (line >= 0){
        ...
        while (x<TET_MAX_BLOCKS_X){
            if (screen[x + (TET_MAX_BLOCKS_X * line)] == BLACK){
...
void remove_line_from_screen(int rmline){
    ...
    x = 0;
    while (x < TET_MAX_BLOCKS_X){
        screen[x + (TET_MAX_BLOCKS_X * 0)] = BLACK;
        x++;
    }
}
...
void new_game (int level){
    ...
    // clear the screen
    lcd.drawRectangle(0, 0, WIDTHPORTRAIT-1, HEIGHTPORTRAIT-1, BACKGROUND, FILL);
...
void draw_block(int x, int y, int clr){
    lcd.drawRectangle(1 + (x * length), y * length, (x + 1) * length, (y + 1) * length, clr, FILL);
}
...
int touching_top(){
    ...
    while (i < TET_MAX_BLOCKS_X){ //*2
        if (screen[i + (TET_MAX_BLOCKS_X * j)] != BLACK){
            return 1;
        }
        i++;
    }
...
void settings_loop(){
    ...
    while(1){
        // LEVEL
        lcd.drawRectangle(95,20,120,40,BLACK,FILL); //erase previous number
